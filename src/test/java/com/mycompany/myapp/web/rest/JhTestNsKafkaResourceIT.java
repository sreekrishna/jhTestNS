...
class JhTestNsKafkaResourceIT {
    ...
    void setup() {
        ...
        JhTestNsKafkaResource kafkaResource = new JhTestNsKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/jh-test-ns-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jh-test-ns-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
